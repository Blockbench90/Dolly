...
export const FullStory: React.FC = (): React.ReactElement | null => {
    ...
    const isLoading = useSelector(selectIsStoryLoading);
    ...
    React.useEffect(() => {
        ...
        return () => {
            dispatch(setStoryData(undefined));
        };
    }, [dispatch, id]);
                ...
                <Paper className={classes.fullStory}>
                    <div className={classNames(classes.storyHeaderUser)}>
                        <Avatar
                            ...
                            alt={`Аватарка пользователя ${storyData.user.fullname}`}
                            src={storyData.user.avatarUrl}
                        />
                        <Typography>
                            <b>{storyData.user.fullname}</b>&nbsp;
                            <div>
                                <span className={classes.storyUserName}>@{storyData.user.username}</span>&nbsp;
                            </div>
                        </Typography>
                    </div>
